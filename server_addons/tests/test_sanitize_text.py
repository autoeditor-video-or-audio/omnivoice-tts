...
from the cloned reference voice. Each case below pins one rewrite
rule so a regression in `sanitize_text` shows up as a clean unit-test
failure rather than as "voice sounded weird in production".
"""
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Pause family -> long prosodic pause (ellipsis).
        (
        ...
        # Unknown tags get dropped (no random vocalisation).
...
def test_sanitize_text_short_circuits_when_no_brackets() -> None:
    """Hot path: most inputs have no `[`; we should not run any regex."""
    sample = "Just a normal sentence."
